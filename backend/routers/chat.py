...
def save_message_to_db(conversation_id: str, role: str, content: str, model: str):
    ...
    try:
        ...
        cursor.execute(
            """
            INSERT INTO chat_history (conversation_id, role, content, model, created_at)
            VALUES (%s, %s, %s, %s, %s)
            """,
            (conversation_id, role, content, model, datetime.datetime.now())
        )
        conn.commit()
        cursor.close()
        conn.close()
...
Mantén un tono profesional pero cercano, como un tutor experimentado."""
            ...
            save_message_to_db(request.conversation_id, "user", request.message, "user")
...
IMPORTANTE: Responde basándote PRINCIPALMENTE en las leyes oficiales del contexto. Cita los artículos específicos (ej: "artículo 195 del TRLGSS"). Si usas información de materiales de estudio, indícalo claramente."""
            ...
            # Save Assistant Message to DB (accumulated)
            # Note: In a real streaming scenario, we'd need to accumulate the full response.
            # For now, we assume the provider loop finished successfully.
            # We can't easily capture the full stream here without modifying the loop logic significantly.
            # A better approach is to save it in the frontend or use a callback.
            # However, for this implementation, we will try to save what we can or skip streaming persistence for now
            # and focus on the non-streaming endpoint or improve this later.
            
            # IMPROVEMENT: Accumulate content in the loop above
            # full_content = ""
            # ... inside loop: full_content += content
            # ... here: save_message_to_db(request.conversation_id, "assistant", full_content, request.provider)
...
async def chat_message(request: ChatRequest):
    ...
    try:
        ...
        # Save to DB (User and Assistant)
        save_message_to_db(request.conversation_id, "user", request.message, "user")
        save_message_to_db(request.conversation_id, "assistant", full_response, MISTRAL_MODEL)
