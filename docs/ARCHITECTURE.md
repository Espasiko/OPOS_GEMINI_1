...
La aplicación no utiliza un *bundler* como Vite o Webpack. En su lugar, carga las dependencias (React, TailwindCSS, etc.) a través de una red de distribución de contenidos (CDN) y utiliza un `importmap` en `index.html` para gestionar los módulos de ES6, lo que simplifica el entorno de desarrollo.
...
*   **Estado Persistente (`usePersistentState`):** Un *custom hook* en `App.tsx` se encarga de la persistencia de datos. Cuando el estado de una herramienta cambia (por ejemplo, se genera un nuevo caso práctico), este hook lo serializa a JSON y lo guarda en `window.localStorage`. Al iniciar la aplicación, el hook lee los datos de `localStorage` para restaurar el estado anterior, permitiendo al usuario continuar donde lo dejó.

*   **Servicio de IA (`geminiService.ts`):** Este es el único punto de contacto con la API de Google Gemini. Todos los componentes de la interfaz de usuario que necesitan generar contenido llaman a las funciones exportadas por este servicio. El servicio se encarga de construir los *prompts*, configurar los parámetros del modelo (como `responseSchema` o `thinkingConfig`) y manejar las respuestas o errores.
