...
const RenderNode: React.FC<{
  node: MindMapNode;
  onNodeUpdate: (id: string, newText: string) => void;
}> = ({ node, onNodeUpdate }) => {
      ...
      {node.children && node.children.length > 0 && (
        <ul className="pt-4">
          {node.children.map(child => (
            <RenderNode key={child.id} node={child} onNodeUpdate={onNodeUpdate} />
          ))}
        </ul>
      )}
    </li>
  ...
  const downloadAs = (format: 'json' | 'md' | 'png') => {
    ...
    if (format === 'png') {
      if (graphContainerRef.current) {
        htmlToImage
          .toPng(graphContainerRef.current, { backgroundColor: '#f8fafc' }) // bg-slate-50
          .then(dataUrl => {
    ...
    if (format === 'json') {
      dataStr =
        'data:text/json;charset=utf-8,' + encodeURIComponent(JSON.stringify(mindMap, null, 2));
    } else if (format === 'md') {
            ...
            <div className="relative group">
              ...
              <div className="absolute right-0 mt-2 w-40 bg-white dark:bg-slate-800 rounded-md shadow-lg py-1 z-10 opacity-0 group-hover:opacity-100 transition-opacity invisible group-hover:visible">
                <a
                  ...
                  className="block px-4 py-2 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer"
                >
                  Como Imagen (.png)
                </a>
                <a
        ...
        <div className="max-w-6xl mx-auto w-full flex flex-col">
          <div className="flex gap-4 mb-8">
            ...
            <button
              ...
              className="px-4 py-2 bg-blue-500 text-white rounded-lg font-semibold hover:bg-blue-600 disabled:bg-slate-400 flex items-center gap-2"
            >
              {isLoading ? (
          ...
          <p className="text-xs text-center text-slate-500 mt-2">
            Doble clic en un nodo para editar.
          </p>
        </div>
      </div>
    </div>
