      ...
      <div className="space-y-3">
        {question.options.map(option => (
          <div
            ...
            className={`p-4 border dark:border-slate-700 rounded-lg transition-all duration-300 flex items-start space-x-4 ${getOptionClasses(option)}`}
          >
            <div className="font-bold text-slate-700 dark:text-slate-200 flex-shrink-0">
              {option.id}.
            </div>
 ...
 */
const CaseGeneratorView: React.FC<CaseGeneratorViewProps> = ({
  currentCase,
  setCurrentCase,
  caseAnswers,
  setCaseAnswers,
  ...
  const handleOptionSelect = (questionId: string, optionId: string) => {
    ...
    const isCorrect =
      currentCase?.questions.find(q => q.id === questionId)?.correct_option_id === optionId;
  ...
  const renderContent = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-center">
        ...
        <p className="mt-2 text-slate-500 dark:text-slate-400">
          Haz clic en "Nuevo Caso" para generar un supuesto práctico único y desafiante.
        </p>
      </div>
    );
  };
      ...
      <header className="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Casos prácticos con múltiples preguntas, como en el examen oficial.
          </p>
        </div>
        <button
