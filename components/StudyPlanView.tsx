        ...
        <aside className="col-span-1 md:border-r border-slate-200 dark:border-slate-800 p-6 flex flex-col gap-6 bg-slate-50 dark:bg-slate-900/50 overflow-y-auto">
          ...
          <div className="space-y-3">
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={inputs.includeTracking}
                onChange={e => setInputs(prev => ({ ...prev, includeTracking: e.target.checked }))}
                className="rounded"
              />
            ...
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={inputs.includeSuggestions}
                onChange={e =>
                  setInputs(prev => ({ ...prev, includeSuggestions: e.target.checked }))
                }
                className="rounded"
              />
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full mt-auto px-4 py-2 bg-blue-500 text-white rounded-lg font-semibold hover:bg-blue-600 disabled:bg-slate-400 flex items-center justify-center gap-2"
          >
            {isLoading ? (
