---
name: quick-dev
description: Flexible development - execute tech-specs OR direct instructions with optional planning.
invokable: true
---

Ejecuta el workflow **quick-dev**.

**Descripción**: Flexible development - execute tech-specs OR direct instructions with optional planning.

**Instrucciones de ejecución**:

1. Lee el archivo completo del workflow: `/home/spas/OPOS_GEMINI_1/.bmad/.bmad/bmm/workflows/bmad-quick-flow/quick-dev/workflow.yaml`
2. Sigue TODOS los pasos del workflow en el orden especificado
3. Usa los recursos, templates y datos definidos en el workflow
4. Genera los artefactos en la carpeta de salida configurada

**IMPORTANTE**: NO improvises. Ejecuta el workflow completo tal como está definido.

**Módulo**: bmm
