#!/usr/bin/env python3
"""Generador de 20 Q&A con Groq API (Llama 3.3 70B - ultra rápido)"""
...
print("\n🎯 GENERADOR DE 20 Q&A CON GROQ (Llama 3.3 70B)\n")
...
questions = [p for p in points if p.payload.get("subcategory") == "preguntas"][80:100]
print(f"✅ {len(questions)} preguntas (offset 80-100)\n")

generated_qa = []

def query_groq(prompt, model="llama-3.3-70b-versatile"):
    """Consulta Groq API con Llama 3.3 70B"""
    headers = {
...
output_file = f'dataset_output/qa_groq_llama33_20_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
with open(output_file, 'w', encoding='utf-8') as f:
    json.dump(generated_qa, f, indent=2, ensure_ascii=False)

print(f"\n{'='*60}\n✅ GROQ (Llama 3.3 70B): {len(generated_qa)}/20 Q&A\nArchivo: {output_file}")
