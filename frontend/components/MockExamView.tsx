...
const MockExamView: React.FC<{ addProgressData: (data: ProgressData[]) => void }> = ({
  ...
  const correctCount = exam
    ? Object.keys(answers).filter(
        qId =>
          answers[qId].selectedOptions[0] ===
          exam.questions.find(q => q.id === qId)?.correct_option_id
      ).length
    : 0;
              ...
              <div className="flex flex-wrap gap-2">
                {syllabusTopics.map(topic => (
                  <button
                    ...
                    className={`px-3 py-1 rounded-full text-sm font-medium transition-colors ${selectedTopics.includes(topic) ? 'bg-blue-500 text-white' : 'bg-slate-200 dark:bg-slate-700'}`}
                  >
                    {topic}
                  </button>
                ))}
          ...
          <div className="max-w-4xl mx-auto">
            <div className="sticky top-0 bg-white dark:bg-slate-900 py-4 mb-6 z-10 border-b dark:border-slate-800">
              <div className="flex justify-between items-center">
                ...
                <div className="text-xl font-bold bg-slate-200 dark:bg-slate-700 px-4 py-2 rounded-lg">
                  {formatTime(timeLeft)}
                </div>
              </div>
            </div>
                ...
                <div key={q.id} className="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
                  ...
                  <div className="space-y-2">
                    {q.options.map(opt => (
                      <label
                        key={opt.id}
                        className={`flex items-center space-x-3 p-3 rounded-md cursor-pointer transition-colors ${answers[q.id]?.selectedOptions[0] === opt.id ? 'bg-blue-200 dark:bg-blue-900' : 'hover:bg-slate-200 dark:hover:bg-slate-700'}`}
                      >
                        <input
                          type="radio"
                          name={q.id}
            ...
            <button
              ...
              className="mt-8 w-full py-3 bg-green-600 text-white font-bold rounded-lg hover:bg-green-700"
            >
        ...
        return (
          <div className="max-w-4xl mx-auto text-center">
            ...
            <p
              ...
              style={{ color: score >= 5 ? '#22c55e' : '#ef4444' }}
            >
              {score.toFixed(2)} / 10
            </p>
            <p className="text-lg mb-8">
              {correctCount} de {exam?.questions.length} preguntas correctas.
            </p>
            <div className="mb-8">
              <button
                ...
                return (
                  <div key={q.id} className="p-4 border rounded-lg dark:border-slate-700">
                    ...
                    <details className="mt-2 text-sm">
                      ...
                      <p className="mt-1 p-2 bg-slate-100 dark:bg-slate-800 rounded">
                        {q.explanation}
                      </p>
                    </details>
                  </div>
    ...
    <div className="flex flex-col h-full bg-white dark:bg-slate-900">
      <header className="p-4 border-b border-slate-200 dark:border-slate-800">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Ponte a prueba con exámenes generados por IA.
        </p>
      </header>
