...
const getMotivationalMessage = (percentage: number): { message: string; color: string } => {
  if (isNaN(percentage))
    return {
      message: '¡Completa tu primer caso práctico para empezar a medir tu progreso!',
      color: 'text-slate-500',
    };
  if (percentage >= 85)
    return {
      message: '¡Excelente! Estás en el camino correcto para conseguir tu plaza. ¡Sigue así!',
      color: 'text-green-500',
    };
  if (percentage >= 70)
    return {
      message:
        '¡Muy buen trabajo! Estás por encima del notable. Un último empujón te acerca a la excelencia.',
      color: 'text-emerald-500',
    };
  if (percentage >= 50)
    return {
      message: '¡Vas por buen camino! Has superado el aprobado. La constancia es la clave ahora.',
      color: 'text-yellow-500',
    };
  if (percentage >= 30)
    return {
      message:
        'No te desanimes. Cada error es una oportunidad de aprendizaje. ¡Identifica tus fallos y a por ello!',
      color: 'text-orange-500',
    };
  return {
    message:
      'El comienzo siempre es lo más difícil. Analiza las explicaciones y verás cómo mejoras. ¡Mucho ánimo!',
    color: 'text-red-500',
  };
};
    ...
    <div className="flex flex-col h-full bg-white dark:bg-slate-900">
      <header className="p-4 border-b border-slate-200 dark:border-slate-800">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Analiza tu rendimiento y mantén la motivación alta.
        </p>
      </header>
        ...
        <div className="max-w-4xl mx-auto space-y-8">
          <div className="flex justify-end">
            <select
              ...
              value={filter}
              className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 border border-slate-300 dark:border-slate-600 rounded-md px-2 py-1 text-sm"
            >
          ...
          <div className="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-xl shadow-sm text-center">
            ...
            <p className="text-5xl font-bold text-blue-500">
              {isNaN(successPercentage) ? '-' : `${successPercentage}%`}
            </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
            ...
            <div className="p-4 bg-green-50 dark:bg-green-900/40 rounded-xl shadow-sm">
              ...
              <p className="text-3xl font-bold mt-2 text-green-600 dark:text-green-300">
                {correctAnswers}
              </p>
            </div>
            <div className="p-4 bg-red-50 dark:bg-red-900/40 rounded-xl shadow-sm">
              ...
              <p className="text-3xl font-bold mt-2 text-red-600 dark:text-red-300">
                {incorrectAnswers}
              </p>
            </div>
          </div>
