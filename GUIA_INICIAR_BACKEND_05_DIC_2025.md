...
```powershell
# 1. En PowerShell, cambiar a carpeta frontend
cd frontend
...
Después de iniciar ambos, verifica que todo funciona:
...
Invoke-WebRequest -Uri "http://localhost:5173" | Select-Object -ExpandProperty StatusCode
...
Windows PowerShell
  ...
  │  ├─ npm run dev
  │  └─ Conexión a Backend (http://localhost:8000)
  │
...
- [ ] `npm install` completado en carpeta frontend
- [ ] Frontend iniciado: `npm run dev`
- [ ] `http://localhost:5173` carga correctamente
- [ ] Frontend conecta a Backend (http://localhost:8000)
...
2. Testear que el frontend inicia correctamente
3. Verificar conectividad entre frontend y backend
4. Verificar conectividad con todas las bases de datos
5. Probar endpoints del RAG
